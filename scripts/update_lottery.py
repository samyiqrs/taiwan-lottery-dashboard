...
<main>
<section class="hero"><div class="hero-main"><div class="pill">V2｜回測優先，不假裝神準</div><h1>大樂透與威力彩<br>策略分析</h1><p class="subtitle">官方開獎資料自動更新。推薦邏輯採用策略比較、rolling backtest 與隨機基準；分數是排序訊號，不是中獎機率。</p><div class="tabs" role="tablist" aria-label="彩券種類"><button type="button" class="tab active" role="tab" aria-selected="true" data-game="lotto649">大樂透</button><button type="button" class="tab" role="tab" aria-selected="false" data-game="superlotto">威力彩</button></div><div class="warn">理性提醒：彩券開獎近似獨立隨機事件。若回測沒有穩定勝過隨機基準，就應視為娛樂參考，而不是預測優勢。</div></div><div class="hero-side"><h2>今日結論</h2><p class="small" id="todayConclusion"></p><div class="numbers" id="heroNums" aria-label="推薦號碼"></div><button type="button" class="btn primary" id="copyBtn">複製推薦號碼</button><div class="small copy-status" id="copyStatus" aria-live="polite"></div></div></section>
...
function render(){let x=d();document.querySelectorAll('.tab').forEach(b=>{let active=b.dataset.game===game;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active))});$('period').textContent=x.period;$('date').textContent=x.date;$('tests').textContent=x.backtest.tests+' 期';$('edge').textContent=x.edgeNote;$('todayConclusion').innerHTML=`${x.title} 目前採用 <b>${strategy}</b>。${x.edgeNote}。建議把這當作策略觀察，不是保證預測。`;currentPick=x.recs[strategy]||x.recs['均衡分散'];$('heroNums').innerHTML=balls(currentPick,x.specialRecommend,x.specialLabel);$('lastNums').innerHTML=balls(x.last.numbers,x.last.special,x.specialLabel);$('currentStrategyBadge').textContent=strategy;renderStrategies();renderBacktest();renderScore();renderCost();$('strategyDesc').textContent=x.strategyInfo[strategy]||'';document.title=`${x.title}｜台灣彩券策略儀表板`}
...
function renderBacktest(){let rows=d().backtest.summary; let h='<tr><th>排名</th><th>策略</th><th>平均命中</th><th>≥2碼</th><th>≥3碼</th><th>最佳</th></tr>'; rows.forEach((r,i)=>{h+=`<tr><td class="${i===0?'rank1':''}">${i+1}</td><td>${r.strategy}</td><td>${r.avgHit}</td><td>${r.hit2}%</td><td>${r.hit3}%</td><td>${r.best}</td></tr>`}); $('backtestTable').innerHTML=h}
function renderScore(){let rows=Object.entries(d().scores).sort((a,b)=>b[1].balanced-a[1].balanced).slice(0,18); let h='<tr><th>排名</th><th>號碼</th><th>排序分</th><th>次數</th><th>近15期</th><th>遺漏</th><th>視覺</th></tr>'; rows.forEach(([n,s],i)=>{h+=`<tr><td>${i+1}</td><td><b>${n}</b></td><td>${s.balanced}</td><td>${s.f}</td><td>${s.recent}</td><td>${s.m}</td><td><div class="meter"><i style="width:${s.balanced}%"></i></div></td></tr>`}); $('scoreTable').innerHTML=h}
function renderCost(){let x=d(); $('cost').innerHTML=Object.entries(x.cost).map(([k,v])=>`<div><span>${k} 碼</span><b>${v}</b><small>注</small></div>`).join('')}
async function copyPick(){const text=`${d().title} ${strategy}: ${currentPick.join(', ')}｜${d().specialLabel} ${d().specialRecommend}`;try{await navigator.clipboard.writeText(text);$('copyStatus').textContent='已複製到剪貼簿'}catch(e){const area=document.createElement('textarea');area.value=text;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();document.execCommand('copy');area.remove();$('copyStatus').textContent='已複製到剪貼簿'}setTimeout(()=>{$('copyStatus').textContent=''},2200)}
...
def write_xlsx(data: dict) -> None:
    ...
    overview = [["台灣彩券策略儀表板 V2"], ["更新時間", datetime.now().strftime("%Y-%m-%d %H:%M:%S")]]
    for spec in GAMES:
        g = data[spec.key]
    ...
    with zipfile.ZipFile(XLSX_PATH, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("xl/workbook.xml", '<?xml version="1.0"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>' + ''.join(wb) + '</sheets></workbook>')


def write_audit(data: dict) -> None:
    lines = ["# 台灣彩券分析套件 V2：演算法與 UI 審查", "", f"更新時間：{datetime.now():%Y-%m-%d %H:%M:%S}", "", "## 本次 V2 實際修改", "", "1. 移除原本『AI 深度預測』式定位，改成策略儀表板。", "2. 推薦邏輯不再使用 V1 的『近期未遺漏加分』作為核心公式。", "3. 新增 rolling backtest，用前 50 期預測下一期，並與固定種子的隨機基準比較。", "4. 預設『回測最佳』若只小幅勝過隨機，會回退到均衡分散，不假裝有優勢。", "5. UI 重做為儀表板：今日結論、推薦號碼、最新開獎、回測比較、包牌成本、排序訊號。", "", "## 仍須保守解讀", "", "彩券開獎近似獨立隨機。即使某策略在短期回測略勝，也可能只是噪音；工具只能當娛樂與策略觀察。", ""]
    for spec in GAMES:
        g = data[spec.key]
